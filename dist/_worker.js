...
`.trim()}const rs={definition:["의미","정의","사전","생태학적","개념","이란","무엇","장소"],meaning:["의미","가치","치유","안정","교육적","기능","중요","효과"],activity:["체험","활동","교육","놀이","경험","학습","탐색","참여"]};function xt(t){const e={definition:0,meaning:0,activity:0};for(const[s,r]of Object.entries(rs))for(const i of r)t.includes(i)&&e[s]++;const n=Math.max(e.definition,e.meaning,e.activity);return n===0?null:e.definition===n?"definition":e.meaning===n?"meaning":"activity"}function is(t,e,n){const s=ee(e),r=[],i=new Set,a=/\(([^)]+,?\s*\d{4})\)/g;let o;for(;(o=a.exec(e))!==null;)i.add(o[1]);for(const b of t){const v=[];let _;const I=/\(([^)]+,?\s*\d{4})\)/g;for(;(_=I.exec(b))!==null;){const O=_[1];i.has(O)&&v.push(O)}let g=b.replace(/\(([^)]+,?\s*\d{4})\)/g,"").replace(/^(그리고|또한|따라서|즉|또|한편|이러한|이와같이|결국|그러나|하지만|더불어|아울러)[,\s]*/g,"").replace(/[\.。\?\!]+$/,"").replace(/\(p\.\s*\d+\)/gi,"").replace(/\[[^\]]*\]/g,"").replace(/\s*-\s*\d+\s*-\s*/g," ").replace(/것이\s+다/g,"것이다").replace(/바이\s+다/g,"바이다").replace(/직\s+접/g,"직접").replace(/만나\s+게/g,"만나게").replace(/자유롭\s+게/g,"자유롭게").replace(/\s{2,}/g," ").trim();if(g.length<10)continue;const D=Ze(g).slice(0,8);r.push({original:b,clean:g,keywords:D,citations:v}),g.includes("(")&&console.log("[DEBUG] 인용 미제거:",g.slice(0,100))}if(r.length===0)return"요약할 내용이 부족합니다.";const c=new Map;for(const b of r)for(const v of b.keywords)c.set(v,(c.get(v)||0)+1);const l=[];for(const b of r){new Set(b.keywords);let v=!1;for(const _ of l)if(b.keywords.filter(g=>_.keywords.has(g)).length>=2){_.sentences.push({clean:b.clean,citations:b.citations}),b.keywords.forEach(g=>_.keywords.add(g)),v=!0;break}v||l.push({keywords:new Set(b.keywords),sentences:[{clean:b.clean,citations:b.citations}]})}const u=l.map(b=>{const v=b.sentences[0].clean,_=r.findIndex(I=>I.clean===v);return{...b,originalIdx:_}});let h="";if(n==="brief"){const b={definition:[],meaning:[],activity:[]};for(const m of u)for(const S of m.sentences){const N=xt(S.clean);N&&b[N].push(S)}const v=b.definition[0],_=b.meaning[0],I=b.activity[0],g=[],D=[];if(v&&(g.push(v.clean),D.push(...v.citations.filter(Boolean))),_&&(g.push(_.clean),D.push(..._.citations.filter(Boolean))),I&&(g.push(I.clean),D.push(...I.citations.filter(Boolean))),g.length===0){const S=u.sort((N,P)=>P.sentences.length-N.sentences.length)[0].sentences[0];g.push(S.clean),D.push(...S.citations.filter(Boolean))}const O=Array.from(new Set(D)),x=O.length>0?`(${O.join("; ")})`:"",f=g.map(m=>{let S=m;for(;S.includes("(");)S=S.replace(/\([^)]*\)/g,"");return S.trim()});f.length===1?h=`${f[0]}${x}.`:f.length===2?h=`${f[0]}. ${f[1]}${x}.`:h=`${f[0]}하며 ${f[1]}. ${f[2]}${x}.`;const E=ee(h)/s*100;if(E>15){let m=h.slice(0,60);m=m.replace(/\([^)]*\)/g,"").trim(),h=m+(x?` ${x}.`:".")}const p=[];return v&&p.push("definition"),_&&p.push("meaning"),I&&p.push("activity"),typeof console<"u"&&console.log("[Brief Summary Meta]",{rolesFilled:p,sentenceCount:g.length,compressionRatio:E.toFixed(1)+"%",passed:E<=15}),h=at(e,h,"brief",t),h}if(n==="standard"){const b=u.sort((m,S)=>S.sentences.length-m.sentences.length).slice(0,3).sort((m,S)=>m.originalIdx-S.originalIdx);if(b.length===1){const m=b[0].sentences[0],S=b[0].sentences.flatMap(P=>P.citations).filter(Boolean),N=S.length>0?`(${S.join("; ")})`:"";return`${m.clean}${N}.`}const v=new Map,_=new Map,I={오감:["감각","감각적 경험","직접 체험"],탐색:["탐구","관찰","발견"],체험:["경험","활동","학습"],자연:["숲","환경","생태계"]};for(const m of b)for(const S of m.sentences){const N=S.clean.match(/^(.+?)[은는이가]\s*(.+)$/);if(N){let[,P,q]=N;P=P.replace(/[에게서로부터]$/g,"").trim(),v.has(P)||v.set(P,[]);let G=q.trim();G=G.replace(/[\.。\?\!]+$/g,"").trim();for(const[B,pe]of Object.entries(I))if(G.includes(B)){const oe=_.get(B)||0;if(_.set(B,oe+1),oe>=1&&pe.length>0){const Ie=Math.min(oe-1,pe.length-1);G=G.replace(B,pe[Ie])}}const L=new Set(Ze(G)),H=es(L),W=new Set(["오감","감각","감각적","체험","경험","활동","학습"]);for(const B of W)H.delete(B);v.get(P).push({original:G,keywords:H,citations:S.citations})}}const g=[];for(const[m,S]of v.entries()){const N=S.flatMap(L=>L.citations).filter(Boolean),P=m.charAt(m.length-1),G=/[가-힣]/.test(P)&&(P.charCodeAt(0)-44032)%28!==0?"은":"는";if(S.length===1){const L=S[0].original,H=(L.match(/,/g)||[]).length;if(L.length>80&&H>=2){const W=L.split(",").map(B=>B.trim()).filter(B=>B.length>0);if(W.length>=2){g.push({text:`${m}${G} ${W[0]}`,citations:[]});for(let B=1;B<W.length-1;B++)g.push({text:`${W[B]}`,citations:[]});g.push({text:`${W[W.length-1]}`,citations:S[0].citations})}else g.push({text:`${m}${G} ${L}`,citations:N})}else g.push({text:`${m}${G} ${L}`,citations:N})}else{const L=[];for(const H of S){let W=!1;for(const B of L){const pe=Array.from(H.keywords).filter(Ie=>B.keywords.has(Ie)).length,oe=Math.max(H.keywords.size,B.keywords.size);if(oe>0&&pe/oe>=.8){H.original.length>B.original.length&&(B.original=H.original,B.keywords=H.keywords),B.citations.push(...H.citations),W=!0;break}}W||L.push({original:H.original,keywords:H.keywords,citations:[...H.citations]})}if(L.length===1)g.push({text:`${m}${G} ${L[0].original}`,citations:L.flatMap(H=>H.citations)});else if(L.length===2)g.push({text:`${m}${G} ${L[0].original}`,citations:L[0].citations}),g.push({text:`${m}${G} ${L[1].original}`,citations:L[1].citations});else for(let H=0;H<L.length;H++)g.push({text:`${m}${G} ${L[H].original}`,citations:L[H].citations})}}if(g.length===0)return"요약할 내용이 부족합니다.";if(g.length===1){const m=g[0].citations.filter(Boolean),S=m.length>0?`(${m.join("; ")})`:"";return`${g[0].text}${S}.`}if(g.length===2){const m=g[0].citations.filter(Boolean),S=g[1].citations.filter(Boolean),N=m.length>0?`(${m.join("; ")})`:"",P=S.length>0?`(${S.join("; ")})`:"";return`${g[0].text}${N}. ${g[1].text}${P}.`}const D=[],O=g[0],x=O.citations.filter(Boolean),f=x.length>0?`(${x.join("; ")})`:"";if(D.push(`${O.text}${f}.`),g.length>=2){const m=g[1],S=m.citations.filter(Boolean),N=S.length>0?`(${S.join("; ")})`:"";D.push(`${m.text}${N}.`)}if(g.length>=3){const S=g.slice(2).map(N=>{const P=N.citations.filter(Boolean),q=P.length>0?`(${P.join("; ")})`:"";return`${N.text}${q}.`});D.push(S.join(" "))}h=D.join(`

`);const E=ee(h)/s*100;E>30&&(D.length>3?h=D.slice(0,3).join(`

`):h=D.join(`

`));const p=[];for(const m of b)for(const S of m.sentences){const N=xt(S.clean);N&&!p.includes(N)&&p.push(N)}return typeof console<"u"&&console.log("[Standard Summary Meta]",{rolesFilled:p,sentenceCount:g.length,paragraphCount:D.length,compressionRatio:E.toFixed(1)+"%",passed:E>=25&&E<=30}),h=at(e,h,"standard",t),h}const w=u.sort((b,v)=>v.sentences.length-b.sentences.length).slice(0,5).sort((b,v)=>b.originalIdx-v.originalIdx);let T=w.map((b,v)=>{const _=b.sentences[0],I=b.sentences.flatMap(D=>D.citations).filter(Boolean),g=I.length>0?`(${I.join("; ")})`:"";return v===0?`${_.clean}${g}.`:v===w.length-1?`마지막으로 ${_.clean}${g}.`:`또한 ${_.clean}${g}.`}).join(" ");return ee(T)/s*100>(n==="brief"?15:n==="standard"?30:55)&&n==="detail"?w.slice(0,3).map((v,_)=>{const I=v.sentences[0],g=v.sentences.flatMap(O=>O.citations).filter(Boolean),D=g.length>0?`(${g.join("; ")})`:"";return _===0?`${I.clean}${D}.`:_===2?`마지막으로 ${I.clean}${D}.`:`또한 ${I.clean}${D}.`}).join(" "):(T=at(e,T,"detail",t),T)}function as(t,e,n){const s=Wt(t),r=e==="brief"?Ee(Math.round(s.length*.18),2,4):e==="standard"?Ee(Math.round(s.length*.28),4,8):Ee(Math.round(s.length*.4),7,14),i=ns(s,r);if(n==="narrative"){let o=is(i,t,e);return o=Vt(o),{kind:"summary",mode:e,viewType:n,narrative:o}}if(n==="structured")return{kind:"summary",mode:e,viewType:n,structured:{title:"구조화 요약",bullets:i.map((o,c)=>`- (${c+1}) ${o}`)}};if(n==="mindmap"){const o=(i[0]||s[0]||"핵심").slice(0,40),c=[{id:"c",label:o,level:0}],l=[];return i.slice(1).forEach((u,h)=>{const w=`n${h+1}`;c.push({id:w,label:u.slice(0,60),level:1}),l.push({from:"c",to:w})}),{kind:"summary",mode:e,viewType:n,mindmap:{center:o,nodes:c,edges:l}}}const a=i.map((o,c)=>({id:`q${c+1}`,type:"short",question:`(${c+1}) 다음 내용을 한 문장으로 설명해보세요: "${o.slice(0,70)}"`,answerHint:o}));return{kind:"summary",mode:e,viewType:n,selftest:{title:"셀프테스트",questions:a}}}function Qt(t){if(!t)return"empty";let e=2166136261,n=0;for(let i=0;i<t.length;i++){const a=t.charCodeAt(i);e^=a,e+=(e<<1)+(e<<4)+(e<<7)+(e<<8)+(e<<24),n=(n<<5)-n+a,n|=0}const s=(e>>>0).toString(16),r=(Math.abs(n)>>>0).toString(16);return`${t.length.toString(16)}_${s}_${r}`}function os(t,e,n,s){const r=Qt(n);return`${t}::${s||"anon"}::${e}::base::${r}`}function cs(t,e,n,s,r){const i=Qt(s);return`${t}::${r||"anon"}::${e}::${n}::${i}`}async function ls(t){if(!it){if(!t){it=!0;return}await t.prepare(`CREATE TABLE IF NOT EXISTS summary_cache (
      cache_key TEXT PRIMARY KEY,
      user_id TEXT NOT NULL,
      response_json TEXT NOT NULL,
      created_at TEXT NOT NULL
    );`).run(),await t.prepare(`CREATE INDEX IF NOT EXISTS idx_summary_cache_user_created
     ON summary_cache(user_id, created_at);`).run(),it=!0}}async function bt(t,e){const n=Date.now(),s=Ve.get(e);if(s&&n-s.createdAt<Wn)return{hit:!0,data:s.data,store:"mem"};if(s&&Ve.delete(e),!t)return{hit:!1};const r=await t.prepare("SELECT response_json, created_at FROM summary_cache WHERE cache_key=?").bind(e).first();if(!(r!=null&&r.response_json))return{hit:!1};try{const i=JSON.parse(r.response_json);return Ve.set(e,{data:i,createdAt:n}),{hit:!0,data:i,store:"d1"}}catch{return{hit:!1}}}async function Le(t,e,n,s){const r=Date.now();Ve.set(e,{data:s,createdAt:r}),t&&await t.prepare(`INSERT OR REPLACE INTO summary_cache(cache_key, user_id, response_json, created_at)
     VALUES (?,?,?,?)`).bind(e,n,JSON.stringify(s),Ft()).run()}function vt(t){const e=t.split(/\n\n+/).filter(s=>s.trim());return{kind:"summary",viewType:"structured",structured:{title:"구조화 요약",bullets:e.length>1?e.map((s,r)=>`- (${r+1}) ${s}`):t.split(/[\.。]\s+/).filter(s=>s.trim()).map((s,r)=>`- (${r+1}) ${s}.`)}}}function wt(t){const e=t.split(/[\.。]\s+/).filter(i=>i.trim()).map(i=>i.trim()),n=(e[0]||"핵심").slice(0,40),s=[{id:"c",label:n,level:0}],r=[];return e.slice(1).forEach((i,a)=>{const o=`n${a+1}`;s.push({id:o,label:i.slice(0,60),level:1}),r.push({from:"c",to:o})}),{kind:"summary",viewType:"mindmap",mindmap:{center:n,nodes:s,edges:r}}}function yt(t){return{kind:"summary",viewType:"selftest",selftest:{title:"셀프테스트",questions:t.split(/[\.。]\s+/).filter(s=>s.trim()).map(s=>s.trim()).map((s,r)=>({id:`q${r+1}`,type:"short",question:`(${r+1}) 다음 내용을 한 문장으로 설명해보세요: "${s.slice(0,70)}"`,answerHint:s}))}}}async function ds(t,e){var c,l,u,h,w;const n=F(t.GEMINI_API_KEY).trim();if(!n)throw new Error("GEMINI_API_KEY is missing");const s=F(t.GEMINI_MODEL).trim()||"gemini-1.5-flash",r=`https://generativelanguage.googleapis.com/v1beta/models/${encodeURIComponent(s)}:generateContent?key=${encodeURIComponent(n)}`,i={contents:[{role:"user",parts:[{text:e}]}],generationConfig:{temperature:.3,topP:.9,maxOutputTokens:2048,topK:40},safetySettings:[{category:"HARM_CATEGORY_HARASSMENT",threshold:"BLOCK_NONE"},{category:"HARM_CATEGORY_HATE_SPEECH",threshold:"BLOCK_NONE"},{category:"HARM_CATEGORY_SEXUALLY_EXPLICIT",threshold:"BLOCK_NONE"},{category:"HARM_CATEGORY_DANGEROUS_CONTENT",threshold:"BLOCK_NONE"}]};let a=0,o=500;for(;a<3;){a++;const T=await fetch(r,{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify(i)});if(T.ok){const k=await T.json();return{ok:!0,text:((w=(h=(u=(l=(c=k==null?void 0:k.candidates)==null?void 0:c[0])==null?void 0:l.content)==null?void 0:u.parts)==null?void 0:h[0])==null?void 0:w.text)??"",raw:k}}if(T.status===429||T.status===503){await new Promise(k=>setTimeout(k,o)),o*=2;continue}const M=await T.text().catch(()=>"");throw new Error(`Gemini error ${T.status}: ${M.slice(0,200)}`)}throw new Error("Gemini retry exceeded")}async function us(t,e,n){var l,u,h,w,T;const s=F(t.GEMINI_API_KEY).trim();if(!s)throw new Error("GEMINI_API_KEY is missing");const r=F(t.GEMINI_MODEL).trim()||"gemini-1.5-flash",i=`https://generativelanguage.googleapis.com/v1beta/models/${encodeURIComponent(r)}:generateContent?key=${encodeURIComponent(s)}`,a={system_instruction:{parts:[{text:e}]},contents:[{role:"user",parts:[{text:n}]}],generationConfig:{temperature:.3,topP:.9,maxOutputTokens:2048,topK:40},safetySettings:[{category:"HARM_CATEGORY_HARASSMENT",threshold:"BLOCK_NONE"},{category:"HARM_CATEGORY_HATE_SPEECH",threshold:"BLOCK_NONE"},{category:"HARM_CATEGORY_SEXUALLY_EXPLICIT",threshold:"BLOCK_NONE"},{category:"HARM_CATEGORY_DANGEROUS_CONTENT",threshold:"BLOCK_NONE"}]};let o=0,c=500;for(;o<3;){o++;const M=await fetch(i,{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify(a)});if(M.ok){const A=await M.json();return((T=(w=(h=(u=(l=A==null?void 0:A.candidates)==null?void 0:l[0])==null?void 0:u.content)==null?void 0:h.parts)==null?void 0:w[0])==null?void 0:T.text)??""}if(M.status===429||M.status===503){await new Promise(A=>setTimeout(A,c)),c*=2;continue}const k=await M.text().catch(()=>"");throw new Error(`Gemini error ${M.status}: ${k.slice(0,200)}`)}throw new Error("Gemini retry exceeded")}async function Zt(t,e){const n=await ds(t,e);return typeof n=="string"?n:((n==null?void 0:n.text)??"").toString()}async function hs(t,e){const n=ss(e);for(let s=1;s<=2;s++)try{let i=(await Zt(t,n)||"").trim();i.startsWith("```")&&(i=i.replace(/^```(?:json)?\s*/i,"").replace(/```\s*$/i,"").trim());const a=JSON.parse(i);if(!(a!=null&&a.brief)||!(a!=null&&a.standard)||!(a!=null&&a.detail))throw new Error("Missing required fields");if(!a.detail.개념||!a.detail.영향||!a.detail["교육적 가치"])throw new Error("Missing detail fields");const o=ee(a.brief),c=ee(a.standard),l=ee(a.detail.개념+a.detail.영향+a.detail["교육적 가치"]);return(o>=c||c>=l)&&console.warn("[SummaryJSON] monotonic violated",{bLen:o,sLen:c,dLen:l,attempt:s}),a}catch(r){if(console.error("[SummaryJSON] attempt failed",s,r==null?void 0:r.message),s===2){const i=Xt(e);return{meta:{base_chars_no_space:i.base,target:{brief:i.brief,standard:i.standard,detail:i.detail}},brief:"[JSON 실패] 요약 생성 실패",standard:"[JSON 실패] 요약 생성 실패",detail:{개념:"[실패]",영향:"[실패]","교육적 가치":"[실패]"}}}}throw new Error("summarizeWithJSON failed")}function at(t,e,n,s){const{min:r,max:i}=Xe(t,n);let a=(e||"").trim();const o=()=>ee(a),c=()=>{a=Vt(a),a=a.replace(/\s{2,}/g," ").trim()};if(c(),o()>i){const l=Wt(a);for(;l.length>1&&ee(l.join(" "))>i;)l.pop();a=l.join(" "),c()}if(o()<r){const l=(s||[]).map(u=>u.trim()).filter(Boolean);for(const u of l){if(o()>=r)break;const h=ct(u).slice(0,24);if(!(h&&ct(a).includes(h))&&(a=(a?a+" ":"")+u.replace(/[\.。\?\!]+$/g,"")+".",c(),o()>i))break}}return a}const fs=(()=>{const e={brief:{min:.1,max:.15},standard:{min:.25,max:.3},detail:{min:.45,max:.55}},n={brief:6,standard:10,detail:14},s=["narrative","structured","mindmap"],r=["preview","exam"];function i(O){return(O||"").replace(/\s+/g,"")}function a(O,x){const y=Math.max(200,i(O||"").length),E=e[x]||e.standard,p=Math.floor(y*E.min),m=Math.ceil(y*E.max);return{base:y,min:Math.max(80,p),max:Math.max(120,m)}}function o(O){const x=(O||"").trim();return x?x.replace(/\r/g,"").split(new RegExp("(?<=[\\.\\?\\!])\\s+|\\n+")).map(y=>y.trim()).filter(Boolean):[]}function c(O){return o(O).map((f,y)=>({sid:`S${y+1}`,text:f}))}function l(O,x,f){const y=O.find(E=>E.sid===x);return!y||!f||typeof f!="string"?!1:y.text.includes(f.trim())}function u(){return["당신은 교육공학 기반 요약·셀프테스트 생성 엔진이다.","추출형 복붙 금지. 반드시 의미 단위로 재구성하라.","가장 중요한 1순위는 문자수(공백 제외) 제한 준수다.","허위 정보(원문/요약에 없는 내용) 생성 금지.","JSON 출력이 요구되면 JSON만 출력하라."].join(`
`)}function h({originalText:O,mode:x,format:f}){const y=a(O,x),E=f==="narrative"?"서술형: 연결어를 사용해 흐름/인과가 보이도록 1~3문단으로 구성":f==="structured"?"구조화: 상위-하위 위계가 드러나는 조목(가/나/다 또는 ①②③) 형태":"마인드맵: 텍스트로 표현된 노드-관계 목록(중심노드/하위노드/연결라벨) 형태";return["[TASK] 아래 원문을 지정된 형식으로 요약하라.",`- 모드: ${x} (간단/표준/상세)`,`- 형식: ${f} (${E})`,`- 문자수 목표(공백 제외): 최소 ${y.min}자 ~ 최대 ${y.max}자`,"- 주의: 숫자 맞추기 위해 중간을 자르는 행위 금지. 자연스러운 문장으로 재작성.","- 주의: 원문에 없는 주장/사례/인과 추가 금지.","","[ORIGINAL]",O].join(`
...
`)}function T({mode:O,purpose:x,format:f,summaryText:y,sentTable:E,anchors:p}){const m=n[O]||10,S=x==="preview"?"예습용 셀프테스트: 요약문 안에서 바로 확인 가능한 재인 중심(스키마 형성). 과도한 추론 금지.":"시험대비 셀프테스트: 요약문에 있는 근거를 바탕으로 인과/관계/분류를 인출하는 회상 중심. 요약에 없는 정보 금지.",N=f==="narrative"?"문항 스타일: 문장 빈칸, 문장 순서 배열, 인과관계 단답/서술(요약 근거 필수)":f==="structured"?"문항 스타일: 항목-정의 매칭, 분류 채우기, 상하위 체계, 사례-범주 매칭(요약 근거 필수)":"문항 스타일: 노드 라벨 맞추기, 연결 라벨링, 누락 노드/연결 복원, 관계 이유 단답(요약 근거 필수)";return["[TASK] 아래 요약문과 앵커만을 근거로 셀프테스트 문항을 생성하라.",`- 모드: ${O} (문항수 ${m})`,`- 목적: ${x} (${S})`,`- 요약 형식: ${f} (${N})`,"- 규칙1: 요약문에 없는 정보로 문제 만들지 말 것(할루시네이션 금지).","- 규칙2: 각 문항은 반드시 evidence를 포함: sid + quote(요약 문장 일부 8~25자).","- 규칙3: quote는 반드시 해당 sid 문장에 실제로 포함되어야 한다.","- 규칙4: 정답/해설은 간결하게. 해설은 evidence와 연결되게.","","[OUTPUT JSON ONLY]",`{
  "items":[
    {
      "id":"Q1",
...
`)}function M(O,x){const f=x&&x.anchors?x.anchors:[],y=[],E=[];for(const p of f){const m=p==null?void 0:p.sid,S=p==null?void 0:p.quote;if(typeof(p==null?void 0:p.label)!="string"||!p.label.trim()){E.push({a:p,reason:"label missing"});continue}if(!l(O,m,S)){E.push({a:p,reason:"evidence not in sentence"});continue}y.push(p)}return{ok:y,bad:E}}function k(O,x){const f=x&&Array.isArray(x.items)?x.items:[],y=[],E=[];for(const p of f){const m=p==null?void 0:p.evidence;if(!(p!=null&&p.id)||!(p!=null&&p.question)||!(p!=null&&p.answer)||!(m!=null&&m.sid)||!(m!=null&&m.quote)){E.push({q:p,reason:"missing fields"});continue}if(!l(O,m.sid,m.quote)){E.push({q:p,reason:"evidence not in sentence"});continue}if(Array.isArray(p.choices)&&p.choices.length>0&&!p.choices.includes(p.answer)){E.push({q:p,reason:"answer not in choices"});continue}y.push(p)}return{ok:y,bad:E}}function A({summaryText:O,sentTable:x,anchors:f,badItems:y,mode:E,purpose:p,format:m}){return["[TASK] 아래는 검증에서 탈락한 문항들이다. 요약문 근거(sid+quote)를 만족하도록 문항을 다시 생성하라.",`- 재생성 문항 수: ${y.length}`,`- 모드: ${E}, 목적: ${p}, 형식: ${m}`,"- 규칙: 요약문 밖 정보 금지. 반드시 sid+quote가 실제로 해당 문장에 포함되어야 한다.","- 출력: JSON만. items 길이는 정확히 재생성 문항 수와 같아야 한다.","","[OUTPUT JSON ONLY]",'{"items":[{ "id":"Qx","type":"blank|match|order|label|short|mcq","question":"...","choices":[],"answer":"...","explanation":"...","evidence":{"sid":"S1","quote":"..." },"anchorIds":["A1"] }]}',"","[SUMMARY SENTENCES WITH ID]",JSON.stringify(x,null,2),"","[ANCHORS]",JSON.stringify(f,null,2),"","[BAD ITEMS]",JSON.stringify(y,null,2),"","[SUMMARY]",O].join(`
`)}async function b({llmCall:O,originalText:x,mode:f,format:y}){if(!O)throw new Error("llmCall is required");e[f]||(f="standard"),s.includes(y)||(y="narrative");const E=h({originalText:x,mode:f,format:y}),p=(await O({system:u(),user:E,json:!1})||"").trim()||"",m=c(p),S=w({summaryText:p,format:y});let N=await O({system:u(),user:S,json:!0}),P;try{P=JSON.parse(N)}catch{P={anchors:[]}}const{ok:q}=M(m,P),G=q.length>=4?q:v(m);return{summaryText:p,sentTable:m,anchors:G}}function v(O){const x=[];for(let f=0;f<Math.min(8,O.length);f++){const y=O[f],E=(y.text||"").slice(0,18);x.push({id:`A${f+1}`,label:`문장 핵심${f+1}`,type:"claim",sid:y.sid,quote:E,note:"요약 문장 기반 안전 앵커"})}return x}async function _({llmCall:O,mode:x,purpose:f,format:y,summaryText:E,sentTable:p,anchors:m}){e[x]||(x="standard"),r.includes(f)||(f="preview"),s.includes(y)||(y="narrative");const S=T({mode:x,purpose:f,format:y,summaryText:E,sentTable:p,anchors:m});let N=await O({system:u(),user:S,json:!0}),P;try{P=JSON.parse(N)}catch{P={items:[]}}let{ok:q,bad:G}=k(p,P);if(G.length>0){const H=A({summaryText:E,sentTable:p,anchors:m,badItems:G.map(Ie=>Ie.q),mode:x,purpose:f,format:y});let W=await O({system:u(),user:H,json:!0}),B;try{B=JSON.parse(W)}catch{B={items:[]}}const pe=k(p,B);q=q.concat(pe.ok);const oe=n[x]||10;q=q.slice(0,oe)}else{const H=n[x]||10;q=q.slice(0,H)}const L=n[x]||10;if(q.length<L){const H=I({sentTable:p,anchors:m,count:L-q.length,format:y,purpose:f});q=q.concat(H).slice(0,L)}return{items:q}}function I({sentTable:O,anchors:x,count:f,format:y,purpose:E}){const p=[],m=x.slice(0,Math.max(f,1));for(let S=0;S<f;S++){const N=m[S%m.length],P=N.sid,q=N.quote;p.push({id:`QF${S+1}`,type:"short",question:E==="preview"?`요약에서 '${q}'가 의미하는 핵심 개념을 한 문장으로 말해보세요.`:`요약에서 '${q}'가 포함된 문장의 핵심 인과/관계를 한 문장으로 인출해보세요.`,choices:[],answer:"(서술형 정답: 사용자 입력 비교는 해설 기반 채점 또는 키워드 채점으로 처리)",explanation:"근거 문장을 다시 읽고 핵심을 1문장으로 재구성하면 됩니다.",evidence:{sid:P,quote:q},anchorIds:[N.id]})}return p}class g{constructor(x,{passScore:f=90}={}){this.items=Array.isArray(x)?x:[],this.passScore=f,this.state={idx:0,attempts:1,correct:0,wrongIds:new Set,finished:!1}}gradeAnswer(x,f){if(!x)return{ok:!1,reason:"no item"};const y=x.type;if(y==="mcq"||y==="blank"||y==="match"||y==="order"||y==="label"||y==="short"){if(y==="short")return{ok:!0,reason:"short-auto-pass"};const E=(x.answer||"").trim(),p=(f||"").trim();return{ok:p===E,reason:p===E?"match":"mismatch"}}return{ok:!1,reason:"unknown type"}}getScore(){return this.items.length===0?0:Math.round(this.state.correct/this.items.length*100)}currentItem(){return this.items[this.state.idx]||null}submit(x){if(this.state.finished)return{done:!0,message:"already finished"};const f=this.currentItem();if(this.gradeAnswer(f,x).ok)return this.state.correct+=1,this.next(),{ok:!0,message:"정답 처리",score:this.getScore()};if(this.state.wrongIds.add(f.id),this.state.attempts===1)return this.state.attempts=2,{ok:!1,stage:1,hint:`힌트1: 근거 문장(${f.evidence.sid})을 다시 읽어보세요.`,score:this.getScore()};if(this.state.attempts===2)return this.state.attempts=3,{ok:!1,stage:2,hint:`힌트2: 근거 구절 = '${f.evidence.quote}'`,score:this.getScore()};{const E=f.explanation||"해설 없음";return this.next(),{ok:!1,stage:3,explanation:E,score:this.getScore()}}}next(){if(this.state.idx+=1,this.state.attempts=1,this.state.idx>=this.items.length)if(this.getScore()>=this.passScore)this.state.finished=!0;else{const f=this.items.filter(y=>this.state.wrongIds.has(y.id));this.items=f.length>0?f:this.items,this.state.idx=0,this.state.attempts=1,this.state.correct=0,this.state.wrongIds=new Set}}status(){return{idx:this.state.idx,total:this.items.length,score:this.getScore(),passScore:this.passScore,finished:this.state.finished}}}async function D({llmCall:O,originalText:x,mode:f,format:y,purpose:E}){const p=await b({llmCall:O,originalText:x,mode:f,format:y}),m=await _({llmCall:O,mode:f,purpose:E,format:y,summaryText:p.summaryText,sentTable:p.sentTable,anchors:p.anchors});return{summary:{mode:f,format:y,text:p.summaryText,sentences:p.sentTable,anchors:p.anchors},selfTest:{purpose:E,passScore:90,items:m.items}}}return{computeCharTargets:a,splitSentencesKR:o,makeSentenceTable:c,generateBundle:b,generateSelfTest:_,runPipeline:D,MasteryRunner:g}})(),ps=`/* MindStory Engine Bundle (compat) */
(function(){
...
**교육적 가치**
${v.detail["교육적 가치"]}`;const I={kind:r,mode:a,viewType:"narrative",narrative:_,allSummaries:{brief:v.brief,standard:v.standard,detail:v.detail},meta:v.meta};await Le(n,h,c||"anon",I);let g;return o==="narrative"?g=I:o==="structured"?g={kind:r,mode:a,...vt(_)}:o==="mindmap"?g={kind:r,mode:a,...wt(_)}:g={kind:r,mode:a,...yt(_)},await Le(n,l,c||"anon",g),t.json({ok:!0,data:g,meta:{cached:!1,engine:"gemini-json-v3",elapsedMs:Date.now()-e}},200)}catch(v){console.error("[Gemini JSON Error]",v)}const k=as(i,a,o);if(await Le(n,l,c||"anon",k),k.narrative){const v={kind:"summary",mode:a,viewType:"narrative",narrative:k.narrative};await Le(n,h,c||"anon",v)}return t.json({ok:!0,data:k,meta:{cached:!1,engine:"local",elapsedMs:Date.now()-e}},200)});te.get("/health",t=>t.json({ok:!0,service:"MindStory v2 Revised"}));te.notFound(t=>t.json({ok:!1,error:{code:"NOT_FOUND",message:"Route not found"}},404));const Et=new Ut,gs=Object.assign({"/src/index.tsx":te});let en=!1;for(const[,t]of Object.entries(gs))t&&(Et.route("/",t),Et.notFound(t.notFoundHandler),en=!0);if(!en)throw new Error("Can't import modules from ['/src/index.tsx','/app/server.ts']");export{Et as default};
