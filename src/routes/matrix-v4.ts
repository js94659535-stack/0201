/* =====================================================================
   MINDSTORY MATRIX V4 — KOREAN NARRATIVE PARAPHRASE ENGINE
   파일: /home/user/webapp/src/routes/matrix-v4.ts

   ✅ 핵심 원칙
   1) 서술형(narrative) = 원문 재구성(Paraphrase), 문장 이어붙이기 아님
      - Brief/Standard/Detail이 각각 완전히 다른 표현으로 작성
      - 어휘·어순·문장 구조를 레벨마다 변경
      - 생략부호('...','….','…') 절대 금지
      - 원문에 없는 외부 정보(국가명, 통계, 기관명) 추가 금지
   2) 품질 검증(Quality Gate)
      - 유사도 검사: Brief↔Standard, Standard↔Detail 간 유사도 < 0.7
      - 원문 복사율 검사: 각 레벨의 원문 복사율 < 20%
      - Containment 검사: Brief⊆Standard⊆Detail (문장 포함 관계)
   3) 재생성 루프: 최대 3회 시도, 실패 시 503 에러
   4) Phase2 우선, 실패 시 Phase1 Fallback
===================================================================== */
...
// 주의: 원문에 있는 도메인 키워드는 제외, 원문에 없는 외부 정보만 차단
...
임계값: 유사도 < ${previousMetrics.simThr?.toFixed(2)}, 복사율 < 20%` : ''}`;
  ...
  return [
    ...
    ``,
    `[핵심 정의]`,
    `원문의 가장 중요한 개념을 **완전히 새로운 표현**으로 1-2문장 재구성`,
    `- 원문 문장 구조를 절대 복사하지 말고, 어순과 어휘를 바꿔 작성`,
    ``,
    `[상세 설명]`,
    `왜 중요한지, 어떻게 작동하는지를 **핵심 정의와 다른 표현**으로 2-3문장 작성`,
    `- 핵심 정의에서 사용한 어휘·문장 패턴을 재사용하지 말 것`,
    ``,
    `[결론 및 시사점]`,
    `원문이 시사하는 의미를 **상세 설명과 다른 관점**으로 1-2문장 마무리`,
    `- 상세 설명에서 사용한 접속사·문장 구조를 피할 것`,
    ``,
    `[추가 규칙]`,
...
- "...", "…", "중략", "이하 생략" 금지
- 중복 문장 금지
- 원문에 없는 사실/고유명/숫자/비교(예: 국가명, 통계 등) "새로 만들지 말 것"
...
3. 원문에 명시되지 않은 국가명, 통계, 기관명 등 외부 정보를 절대 추가하지 마라
...
- 말줄임/중략/…/… 금지
- 문장 파손 금지(모든 문장 종결)
- 🚫 절대 금지: 원문에 없는 국가명, 통계(%), 기관명 등 외부 정보 추가 금지!
